...
var msgprogress = 'Retrieving Storage Data';
/*  You may edit the following variables */
var bar_length = 300; // the bar will be this many pixels long
var bar_height = 25; // the bar will be this many pixels high
var bar_color = "white"; // the progress bar color
var bar_background = "white"; // the color of the empty part of bar
var bar_border = "white";  // the color of the bar border
var window_background = "white"; // the color of the pop-up window
var window_border = "gray"; // the border color of pop-up window
...
var bar_perc = Math.round(100 / image_count);
	...
	if (percent < 20) { msgprogress = 'Loading Information'; }
	if (percent >= 20 && percent < 70) { msgprogress = 'Retrieving Data'; }
	if (percent >= 70 && percent < 90) { msgprogress = 'Loading Trend'; }
//	if (percent >= 90 && percent < 95) { msgprogress = 'Looking for Similar Hosts'; }
	if (percent >= 95) { msgprogress = 'Finishing'; }
	// Write the new percent in the progress bar window
	document.getElementById('percent').innerHTML = msgprogress + '...   ' + percent + '%';
	...
	if (j < image_count || j == 0 || doneyet == 0)
		setTimeout('progress_bar();', 1500); 
	else // if done then close the progress bar pop-up window
		setTimeout('close_bar();', 1500); // in half a second
